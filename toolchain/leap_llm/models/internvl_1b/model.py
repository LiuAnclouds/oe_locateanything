...
from leap_llm.models.internvl_1b.blocks import DecoderLayer, EncoderLayer  # noqa: E402
from leap_llm.models.internvl_1b.projector import Projector
...
class LLM(Model):
    def __init__(self, params: LlmConfig, cache_len: int):
        ...
        self.embed_tokens = Embedding(params.vocab_size, params.hidden_size)
        ...
        vocab_size = (
            params.vocab_size
            if params.kept_vocab_size == -1
            else params.kept_vocab_size
        )
        self.lm_head = DynamicQuantLinear(params.hidden_size, vocab_size, bias=False)
        self.params = params
...
class Internlm1b:
    ...
    def load_model(
        ...
        if kept_tokens_file is not None:
            with open(kept_tokens_file, "r", encoding="utf-8") as f:
                kept_tokens = json.load(f)
            config["llm_config"]["kept_vocab_size"] = kept_tokens["token_size"]
        ...
        # process compressed token ids and lm_head
        lm_head_name = "lm_head.weight"
        if lm_head_name in new_state_dict.keys() and kept_tokens is not None:
            token_ids = kept_tokens["token_ids"]
            token_ids.extend(kept_tokens["added_tokens_ids"])
            used_lm_heads = new_state_dict[lm_head_name][token_ids]
            new_state_dict[lm_head_name] = used_lm_heads

        model.load_state_dict(new_state_dict)

        return Internlm1b(model, model_args)
...
class Internvl1bVision:
    ...
    @staticmethod
    @timeit
    def load_model(input_model_path: str, checkpoint: {}) -> "Internvl1bVision":
        config_path = os.path.join(input_model_path, "config.json")
        assert os.path.exists(
            config_path
        ), f"config.json not found in {input_model_path}"
        ...
        for layer_id in range(model_args.num_hidden_layers):
            ...
            wq, wk, wv = Internvl1bVision.split_wqkv(
            ...
            bq, bk, bv = Internvl1bVision.split_bqkv(
        ...
        return Internvl1bVision(model, model_args)
