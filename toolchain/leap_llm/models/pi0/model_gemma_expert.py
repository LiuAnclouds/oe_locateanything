...
from leap_llm.models.pi0.blocks.attention import RotaryPosEmb
from leap_llm.models.pi0.blocks.configuration_gemma import GemmaConfig
from leap_llm.models.pi0.blocks.configuration_paligemma import PaliGemmaConfig
...
class GemmaExpert(Model):
    def __init__(self, config: GemmaConfig):
        ...
        self.action_in_proj = DynamicQuantLinear(32, config.hidden_size)
        self.action_out_proj = DynamicQuantLinear(config.hidden_size, 32)

        self.state_proj = DynamicQuantLinear(32, config.hidden_size)
        ...
        self.idx_id = [i + 1 for i in range(50)]
    ...
    def _build_sinusoidal_lookup_table(
        ...
        # 用 list 存每个 embedding，方便直接取
        return table

    def build(self, state, x_t, attention_mask, position_ids, *caches):
        state_emb = self.state_proj(state)
        state_emb = leap.reshape(state_emb, [1, 1, -1])
        state_emb = leap.tile(state_emb, [1, 14, 1])
        # #TODO: time_val 处理
        # # time_val = leap.get_value_or_create_const(timestep)
        # time_val = timestep.item()

        # idx = int(round((1.0 - timestep) / 0.1))
        for i in range(10):
            time_emb = self.sinusoidal_lookup_table[i : i + 1, :]
            ...
            embs = leap.concat([state_emb, action_time_emb], dim=1)
            ...
            suffix_out = leap.slice(
                outputs_embeds, [0, 14, 0], [1, 64, 1024], [1, 1, 1]
            )
    ...
    def forward(self, state, x_t, attention_mask, position_ids, caches):
        state_emb = self.state_proj(state)
        state_emb = state_emb.repeat(14, 1)
        # time_emb = time_emb[:, None, :].expand_as(action_emb)
        # time_val = timestep.item()
        # idx = int(round((1.0 - time_val) / 0.1))  # 1.0 -> 0, 0.9 -> 1, ..., 0.1 -> 9
        for i in range(10):
            embs = []
            embs.append(state_emb.unsqueeze(0))
            time_emb = self.sinusoidal_lookup_table[i : i + 1, :]
...
class GemmaExpertModel:
    @staticmethod
    @timeit
    def build(model_dir: str, vision_tokens_num) -> "GemmaExpertModel":
        ...
        filtered_state_dict["action_time_mlp_in.weight"] = state_dict[
            "action_time_mlp_in.weight"
        ]
        filtered_state_dict["action_time_mlp_in.bias"] = state_dict[
            "action_time_mlp_in.bias"
        ]
        filtered_state_dict["action_time_mlp_out.weight"] = state_dict[
            "action_time_mlp_out.weight"
        ]
        filtered_state_dict["action_time_mlp_out.bias"] = state_dict[
            "action_time_mlp_out.bias"
        ]
        filtered_state_dict["state_proj.weight"] = state_dict["state_proj.weight"]
        filtered_state_dict["state_proj.bias"] = state_dict["state_proj.bias"]
        ...
        modelConfig.text_config.torch_dtype = "float32"
        ...
        modelConfig.text_config.vision_tokens_num = vision_tokens_num
    ...
    def get_leap_input_types(
        ...
        input_types = [
            ...
            leap.TensorType(
                [1, 1, action_horizon + 14, action_horizon + 14 + token_len],
                leap.float16,
            ),
            leap.TensorType([1, action_horizon + 14], leap.int32),
        ]
    ...
    def compile(
        ...
        **kwargs,
        ...
        inputs = self.get_leap_input_types(
            32, 50, self.model_args.text_config.vision_tokens_num * 3 + 48
        )
