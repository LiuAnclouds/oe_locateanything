...
from leap_llm.models.pi0.blocks.attention import GemmaAttention
...
class GemmaModel(Model):
    def __init__(self, config: GemmaConfig):
        ...
        self.cos = cos[: config.vision_token_num * 3 + 48, :]
        self.sin = sin[: config.vision_token_num * 3 + 48, :]
    ...
    def build(self, tokens, inputs_embeds, attention_mask):
        # embed positions
...
class LanguageModel:
    @staticmethod
    @timeit
    def build(model_dir: str, vision_token_num) -> "LanguageModel":
        ...
        filtered_state_dict["embed_tokens.weight"] = state_dict[
    ...
    def get_leap_input_types(self, seq_len, token_id_len) -> List[leap.TensorType]:
        input_types = [
            ...
            leap.TensorType(
                [1, 1, seq_len + token_id_len, seq_len + token_id_len], leap.float16
            ),
        ]
        return input_types
    ...
    def compile(
        ...
        **kwargs,
        ...
        inputs = self.get_leap_input_types(
            self.model_args.text_config.vision_token_num * 3, 48
        )
