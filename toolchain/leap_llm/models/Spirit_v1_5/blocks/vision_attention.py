...
class Qwen3VLVisionAttention(Module):
    ...
    def build(self, hidden_states, position_embeddings):
        ...
        v_states = leap.transpose(v_states, [0, 2, 1, 3])
        attn_outputs_list = []
        # what can i say? write hard code is fast way to implement the code.
        for i in range(len(self.lengths)):
            q_states_i = leap.slice(q_states, [0, 0, 0 + 320*i, 0], [1, self.num_heads, 320 + 320*i, 64], [1, 1, 1, 1])
            k_states_i = leap.slice(k_states, [0, 0, 0 + 320*i, 0], [1, self.num_heads, 320 + 320*i, 64], [1, 1, 1, 1])
            v_states_i = leap.slice(v_states, [0, 0, 0 + 320*i, 0], [1, self.num_heads, 320 + 320*i, 64], [1, 1, 1, 1])
            attn_wt = leap.mul(self.qk(q_states_i, k_states_i), self.scaling)
            attn_wt = leap.softmax(attn_wt, -1)
            v_states_i = leap.transpose(v_states_i, [0, 1, 3, 2])
            attn_output = self.sv(attn_wt, v_states_i)
            attn_output = leap.transpose(attn_output, [0, 2, 1, 3])
            attn_outputs_list.append(attn_output)
        attn_output = leap.concat(attn_outputs_list, dim=1)
    ...
    def forward(self, hidden_states, position_embeddings):
        ...
        splits = [
            torch.split(tensor, self.lengths, dim=2) for tensor in (query_states, key_states, value_states)
        ]
        attn_outputs = []
        for q, k, v in zip(*splits):

            attn_weights = (
                torch.matmul(q, k.transpose(2, 3)) * self.scaling
            )
