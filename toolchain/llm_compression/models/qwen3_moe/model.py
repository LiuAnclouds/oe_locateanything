...
"""Qwen3Moe TextModel - pure LLM, aligned with transformers Qwen3MoeModel."""
...
def _get_rope_theta(config):
    """Extract rope_theta from config.rope_parameters or config.rope_theta."""
    rp = getattr(config, "rope_parameters", None)
    if rp is not None and isinstance(rp, dict):
        return rp.get("rope_theta", 1000000.0)
    return getattr(config, "rope_theta", 1000000.0)


class Qwen3MoeTextModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.vocab_size = config.vocab_size
        padding_idx = getattr(config, "pad_token_id", None)
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=padding_idx)
    ...
    def forward(
        ...
        input_token_ids = input_token_ids.to(device=position_ids.device)
        inputs_embeds = self.embed_tokens(input_token_ids)
        ...
        cos = cos.squeeze(0)[position_ids]  # (max_len, D)[(B, S)] -> (B, S, D)
