...
from .model import Qwen3_5TextModel, Qwen3_5VisionModel
from .process_utils import generate_func

logger = get_logger(__name__)


class Qwen3_5ModelForGeneration(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.visual = Qwen3_5VisionModel(config.vision_config)
        self.lm = Qwen3_5TextModel(config.text_config)
...
class Qwen3_5(BaseQModel):
    ...
    def build_model(self, model_dir):
        ...
        model_config.pad_token_id = (
            _tokenizer.pad_token_id if _tokenizer.pad_token_id is not None else _tokenizer.eos_token_id
        )
        # Fix: config.text_config.eos_token_id is incorrect (points to pad_token),
        # use the correct value from tokenizer
        model_config.text_config.eos_token_id = _tokenizer.eos_token_id

        model = Qwen3_5ModelForGeneration(model_config)
    ...
    def get_qconfig_setting(self, model_name):
        ...
        if model_name == "visual":
            ...
            for name in self.get_kvcache_names(model_name):
                use_int16 = name.endswith("cache_k_fq")
                module_name_config[name] = {"output": qint16 if use_int16 else qint8}

            use_flash_attention = AttentionManager.is_flash_attn()
            for idx, layer_type in enumerate(cfg.layer_types):
                if layer_type == "full_attention":
                    if use_flash_attention:
                        module_name_config[f"layers.{idx}.self_attn.attention.qk_matmul"] = {"input": [qint8, qint16]}
                        module_name_config[f"layers.{idx}.self_attn.attention.sv_matmul"] = {"input": [qint16, qint8]}
                    else:
                        module_name_config[f"layers.{idx}.self_attn._generated_matmul_0"] = {"input": [qint8, qint16]}
                        module_name_config[f"layers.{idx}.self_attn._generated_matmul_1"] = {"input": [qint16, qint8]}
                elif layer_type == "linear_attention":
                    num_chunks = cfg.max_lm_input_len // 64
                    num_matmuls = 3 + 5 * num_chunks
                    for m in range(num_matmuls):
                        module_name_config[f"layers.{idx}.linear_attn._generated_matmul_{m}"] = {
                            "input": [qint16, qint16]
                        }
