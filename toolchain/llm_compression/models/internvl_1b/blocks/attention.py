...
class InternAttention(nn.Module):
    """Vision encoder attention — split q/k/v proj (no unbind/select)."""
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        query_states = self.q_proj(hidden_states).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        key_states = self.k_proj(hidden_states).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        value_states = self.v_proj(hidden_states).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)

        # pre-scale: scale on Q before matmul (aligned with leap_llm)
        query_states = torch.mul(query_states, self.scale)
...
class InternLM2Attention(nn.Module):
    """LLM decoder attention for InternVL2-1B / InternVL2.5-1B.

    Key differences from Qwen3Attention (InternVL3.5):
    - Has bias on q/k/v proj (bias=True), o_proj bias=False
    - No q_norm/k_norm
    - Uses standard 1D RoPE
    """
    ...
    def __init__(self, config, layer_idx):
        ...
        # InternLM2/Qwen2: q/k/v have bias=True, o_proj has bias=False
        ...
        # No q_norm/k_norm for InternLM2
    ...
    def forward(
        ...
        # No q_norm/k_norm (unlike Qwen3/InternVL3.5)
        ...
        if cache_keys is not None and cache_values is not None:
            ...
            # Key mean subtraction before re-quantization
            # (align with leap_llm build: reduce_mean is after concat, before qk matmul,
            #  and there is no re-quantization of concat result in build)
            if q_len > 1:
                km = key_states.mean(dim=2, keepdim=True)
                key_states = key_states - km
        ...
        attn_weights = torch.mul(attn_weights, self.scaling)
