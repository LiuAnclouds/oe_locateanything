...
class InternVL1BModelForGeneration(nn.Module):
...
class InternVL_1B(BaseQModel):
    ...
    def build_model(self, model_dir):
        ...
        # Create custom model
        model = InternVL1BModelForGeneration(model_config)
        ...
        # Split VIT fused qkv into independent q_proj/k_proj/v_proj
    ...
    def input_preprocess(self, message):
        ...
        # Load and process image, build image token string
        ...
        tokenizer = self.processor
        config = self.generated_model.config
        template_name = getattr(config, "template", "Hermes-2")
    ...
    def get_kvcache_names(self, model_name):
        """Return the module names of KV-cache fake-quant stubs."""
    ...
    def get_qconfig_setting(self, model_name):
        ...
        if model_name == "vision_model":
            ...
            # For decode: sv_int16 uses [qint16, qint8]
            if model_name == "decode":
                for i in range(n_layers):
                    config_mapping[f"layers.{i}.self_attn._generated_matmul_1"] = {"input": [qint16, qint8]}
