...
def torch_chunk_gated_delta_rule(
    ...
    # A = -((k_beta @ key.transpose(-1, -2)) * decay_mask).masked_fill(lower_mask, 0)
    # # Compute (I - A)^{-1} = I + A + A^2 + ... via Neumann series doubling.
    # # A is strictly lower-triangular (nilpotent), so the series terminates at A^{chunk_size}.
    # # ceil(log2(chunk_size)) doublings suffice (e.g. 6 iterations for chunk_size=64).
    # # Each step: T_{k+1} = T_k + A^{2^k} @ T_k, P_{k+1} = A^{2^k} @ A^{2^k}
    # # which doubles the covered power range and avoids any in-place scatter_nd.
    # n_doublings = (chunk_size - 1).bit_length()
    # # Keep clone() here: Horizon QAT observers on the following matmul are not
    # # compatible with the expanded broadcast view from eye(...).expand_as(A).
    # attn = torch.eye(chunk_size, dtype=A.dtype, device=A.device).expand_as(A).clone()
    # P = A
    # for _ in range(n_doublings):
    #     attn_update, P = mm(
    #         torch.stack([P, P], dim=-3), torch.stack([attn, P], dim=-3)
    #     ).unbind(dim=-3)
    #     attn = attn + attn_update
    ...
    # Collect per-chunk outputs in a list and stack afterwards to avoid
    # core_attn_out[:, :, i] = ... which generates a 5-D hbir.scatter_nd
    # (HBIR only supports scatter_nd on 1-D and 2-D tensors).
    chunk_outputs = []
    ...
    # stack: (n_chunks, batch, heads, chunk_size, v_head_dim) → permute → reshape
...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        if use_precomputed_states:
            ...
            mixed_qkv = F.silu(self.conv1d_update(hidden_states_new)[:, :, -seq_len:])
            new_conv_state = hidden_states_new[:, :, -conv_state.shape[-1] :]
        else:
            # When seq_len > conv_kernel_size, the naive pad amount is negative,
            # which PyTorch silently treats as truncation but HBIR rejects
            # (begin must be non-negative). Use explicit slice instead.
        ...
        # empty_state = self._empty_state(hidden_states)
