...
class Qwen3_5MoeRMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-6):
        ...
        self.rmsnorm = RMSNorm(dim, eps=eps)
        self.rmsnorm_1 = RMSNorm(dim, eps=eps, elementwise_affine=False)
    ...
    def forward(self, x):
        ...
        output = self.rmsnorm(x) + self.rmsnorm_1(x)
        return output
...
class Qwen3_5MoeAttention(nn.Module):
    ...
    def forward(
        ...
        if cache_keys is not None and cache_values is not None and cache_keys.numel() > 0:
        ...
        new_key = self.dequant(self.cache_k_fq(new_key))
        new_value = self.dequant(self.cache_v_fq(new_value))

        # empty_state = self._empty_state(hidden_states)
        return attn_output, new_key, new_value, None, None
